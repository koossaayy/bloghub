...
<head>
    ...
    <title>{{ __('BlogHub - Inscription') }}</title>
...
<div class="w-full md:w-1/2 bg-gradient-to-br from-blue-400 to-green-400 p-8 flex flex-col justify-center">            <h2 class="text-4xl font-bold text-white leading-tight mb-6">
                {{ __('Rejoignez la communauté BlogHub.') }}
            </h2>
            <p class="text-white/80 text-lg">
                {{ __('Partagez vos idées, publiez vos articles et connectez-vous avec des lecteurs du monde entier.') }}
            </p>
        </div>

        {{-- Partie droite formulaire --}}
        <div class="w-full md:w-1/2 p-8">
            <h2 class="text-3xl font-bold text-gray-800 mb-2">{{ __('Créer un compte') }}</h2>
            <p class="text-gray-500 mb-8">{{ __('Rejoignez-nous en quelques secondes.') }}</p>
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                <div class="mb-4">
                    <label class="block text-xs font-semibold text-gray-500 uppercase mb-2">{{ __('Mot de passe') }}</label>
                ...
                <div class="mb-6">
                    <label class="block text-xs font-semibold text-gray-500 uppercase mb-2">{{ __('Je suis') }}</label>
                    <select name="role"
                        class="w-full px-4 py-3 bg-gray-100 rounded-lg border-0 focus:outline-none focus:ring-2 focus:ring-blue-400">
                        <option value="lecteur">{{ __('Lecteur') }}</option>
                        <option value="auteur">{{ __('Auteur') }}</option>
                    </select>
                </div>
                <button type="submit"
            ...
            <p class="text-center text-gray-500 mt-6">
                {{ __('Déjà un compte ?') }}
                <a href="{{ route('login') }}" class="text-blue-500 font-semibold hover:underline">{{ __('Connexion') }}</a>
            </p>
        </div>
    </div>
</div>
