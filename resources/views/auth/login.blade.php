...
<head>
    ...
    <title>{{ __('BlogHub - Connexion') }}</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="bg-gray-50 min-h-screen">

<nav class="bg-white px-8 py-4 flex justify-between items-center shadow-sm">
    <a href="{{ route('accueil') }}" class="text-2xl font-bold italic text-gray-800">{{ __('BlogHub') }}</a>
...
<div class="flex items-center justify-center py-16 px-4">
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col md:flex-row w-full max-w-3xl">
        ...
        <div class="w-full md:w-5/12 bg-gradient-to-br from-blue-400 to-green-400 p-8 flex flex-col justify-between">
            <div>
                <h2 class="text-3xl font-bold text-white leading-tight mb-4">
                    {{ __('Redéfinissez votre expérience de lecture.') }}
                </h2>
                <p class="text-white/80">
                    {{ __("Rejoignez une communauté d'esprits curieux et d'écrivains passionnés sur la plateforme éditoriale la plus raffinée du web.") }}
                </p>
            </div>
            <div class="bg-white/20 rounded-xl p-4 flex items-center gap-3 mt-8">
                <img src="https://i.ibb.co/Kzp3Yhsc/CALEBWhats-App-Image-2026-03-20-at-00-49-09.jpg"
                     ...
                     alt="Dassi Caleb">
                <div>
                    <p class="text-white font-medium text-sm">{{ __('"Une interface qui laisse enfin respirer les mots."') }}</p>
                    <p class="text-white/70 text-xs mt-1">{{ __('— Dassi Caleb, Auteur sur BlogHub') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="w-full md:w-7/12 p-8">
            <h2 class="text-2xl font-bold text-gray-800 mb-2">{{ __('Bon retour parmi nous') }}</h2>
            <p class="text-gray-500 mb-6 text-sm">{{ __('Entrez vos identifiants pour accéder à votre espace.') }}</p>
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <div class="mb-6">
                    <div class="flex justify-between items-center mb-2">
                        <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Mot de passe') }}</label>
                        <a href="#" class="text-blue-500 text-xs hover:underline">{{ __('Mot de passe oublié ?') }}</a>
                    </div>
            ...
            <p class="text-center text-gray-500 mt-6 text-sm">
                ...
                <a href="{{ route('register') }}" class="text-blue-500 font-semibold hover:underline">{{ __('Inscription') }}</a>
            </p>
        </div>
    </div>
</div>
...
<footer class="text-center py-6">
    <p class="text-2xl font-bold italic text-gray-800 mb-3">{{ __('BlogHub') }}</p>
    ...
    <p class="text-gray-400 text-xs">{{ __('2026 BlogHub. Tous droits réservés.') }}</p>
</footer>
